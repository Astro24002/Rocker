  ...
  return (
    <section className="settings-view">
      ...
      <div className="settings-list">
        ...
        <SettingRow title="Terminal font" description="Used by all new terminal tabs."><select value={settings.terminalFont} onChange={(event) => onUpdate({ terminalFont: event.target.value })}><option>JetBrains Mono</option><option>SFMono-Regular</option><option>Consolas</option></select></SettingRow>
        ...
        <SettingRow title="Port scan interval" description="Refresh remote listening ports while connected."><select value={settings.portScanInterval} onChange={(event) => onUpdate({ portScanInterval: Number(event.target.value) })}><option value="0">Manual only</option><option value="15">15 seconds</option><option value="30">30 seconds</option><option value="60">1 minute</option></select></SettingRow>
